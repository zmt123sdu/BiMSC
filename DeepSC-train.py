...
channel = 'Rayleigh'
config_number = 0
d_channel = 28  # rate = d_channel / 2
device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")

parser = get_train_parser(case='DeepSC', channel=channel, config_number=config_number, d_channel=d_channel)
...
def train_eval(model, train_dataloader, eval_dataloader, device, num_epoch, lr, optim, init, scheduler_type, save_interval, eval_interval, save_path, resume=""):
    ...
    for epoch_index in range(start_epoch, num_epoch + 1):
        ...
        for sents in train_bar:
            ...
            batch_loss_train = train_step(model=model, sent=sents, pad=pad_idx, end=end_idx, snr=snr, opt=optimizer, loss_function=loss_fn, channel=args.channel, device=device)
        ...
        if epoch_index % eval_interval == 0:
            ...
            with torch.no_grad():
                for sents_eval in eval_bar:
                    ...
                    batch_loss_eval = eval_step(model=model, sent=sents_eval, pad=pad_idx, end=end_idx, snr=snr, loss_function=loss_fn, channel=args.channel, device=device)
...
if __name__ == '__main__':
    ...
    # 创建网络模型
    model = DeepSC(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model, channel_dim=d_channel,
