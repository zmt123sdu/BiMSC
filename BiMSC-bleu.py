...
from utils.utils_for_BiMSC import greedy_decode
...
parser = get_test_parser(case='BiMSC', channel=channel, train_config_number=train_config_number, test_config_number=test_config_number,
        ...
        for loop in range(args.test_loop):
            ...
            for snr in SNR:
                ...
                for sents_test in test_bar:
                    ...
                    snr_extend = snr * torch.ones(sents_test.size(0), 1).to(device)

                    out = greedy_decode(model=model, sent=sents_test, pad=pad_idx, end=end_idx, snr=snr_extend, len_max=args.len_max, start_symbol=start_idx,
                                        channel=args.channel, rate=rate, device=device, threshold=args.threshold)
