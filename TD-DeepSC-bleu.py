...
from utils.utils_for_TD_DeepSC import greedy_decode
...
parser = get_test_parser(case='TD-DeepSC', channel=channel, train_config_number=train_config_number, test_config_number=test_config_number,
            ...
            for snr in SNR:
                ...
                for sents_test in test_bar:
                    ...
                    out = greedy_decode(model=model, sent=sents_test, pad=pad_idx, end=end_idx, snr=snr_extend,
                                        len_max=args.len_max, start_symbol=start_idx, channel=args.channel, rate=rate, device=device)
...
if __name__ == '__main__':
    ...
    model = DeepSC(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model,
                   channel_dim=args.d_channel_max, num_heads=args.num_heads, dim_feedforward=args.dff, dropout=args.dropout).to(device)
