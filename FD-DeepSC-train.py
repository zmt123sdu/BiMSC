...
@File    ：FD-DeepSC-train.py
@Author  ：Mingtong Zhang
@Date    ：2024/2/20 19:59 
'''
import os
import pickle
import time
...
from utils.utils_for_FD_DeepSC import train_step, eval_step
...
parser = get_train_parser(case='FD-DeepSC', channel=channel, config_number=config_number)
        ...
        if epoch_index % eval_interval == 0:
            ...
            with torch.no_grad():
                for sents_eval in eval_bar:
                    ...
                    batch_loss_eval = eval_step(model=model, sent=sents_eval, pad=pad_idx, end=end_idx, snr=snr, loss_function=loss_fn,
                                                channel=args.channel, rate_min=args.rate_min, rate_max=args.rate_max, device=device)
