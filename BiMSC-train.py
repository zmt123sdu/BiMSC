...
from models.BiMSC import BiMSC
from utils.utils_general import setup_seed, Timer, initNetParams, Animator, Data_savemat
from utils.utils_for_BiMSC import train_step, eval_step
...
parser = get_train_parser(case='BiMSC', channel=channel, config_number=config_number)
...
def train_eval(model, train_dataloader, eval_dataloader, device, num_epoch, lr, optim, init, scheduler_type, save_interval, eval_interval, save_path, resume):
    ...
    for epoch_index in range(start_epoch, num_epoch + 1):
        ...
        fanet_mode = True if (epoch_index > args.stage1_epoch) else False
        for sents in train_bar:
            ...
            batch_loss_train = train_step(model=model, sent=sents, pad=pad_idx, end=end_idx, snr=snr, opt=optimizer, loss_function=loss_fn, channel=args.channel,
                                          rate_min=args.rate_min, rate_max=args.rate_max, device=device, threshold=args.threshold, fanet_mode=fanet_mode)
        ...
        if epoch_index % eval_interval == 0:
            ...
            with torch.no_grad():
                for sents_eval in eval_bar:
                    ...
                    batch_loss_eval = eval_step(model=model, sent=sents_eval, pad=pad_idx, end=end_idx, snr=snr, loss_function=loss_fn, channel=args.channel,
                                                rate_min=args.rate_min, rate_max=args.rate_max, device=device, threshold=args.threshold, fanet_mode=fanet_mode)
...
if __name__ == '__main__':
    ...
    # 创建网络模型
    model = BiMSC(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model, channel_dim=args.d_channel_max,
                  num_heads=args.num_heads, dim_feedforward=args.dff, dropout=args.dropout, mlp_ratio=args.mlp_ratio).to(device)
