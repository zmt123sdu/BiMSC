...
d_channel = 20  # rate = d_channel / 2
test_rate_list = [d_channel / 2]

device = torch.device("cuda:1" if torch.cuda.is_available() else "cpu")

parser = get_test_parser(case='DeepSC', channel=channel, train_config_number=train_config_number, test_config_number=test_config_number,
                         trained_epoch=trained_epoch, best_mode=best_mode, test_rate_list=test_rate_list, d_channel=d_channel)
        ...
        for loop in range(args.test_loop):
            ...
            for snr in SNR:
                ...
                for sents_test in test_bar:
                    ...
                    out = greedy_decode(model=model, sent=sents_test, pad=pad_idx, end=end_idx, snr=snr_extend, len_max=args.len_max,
                                        start_symbol=start_idx, channel=args.channel, device=device)
...
if __name__ == '__main__':
    ...
    model = DeepSC(num_layers=args.num_layers, src_vocab_size=num_vocab, tgt_vocab_size=num_vocab, model_dim=args.d_model,
                   channel_dim=d_channel, num_heads=args.num_heads, dim_feedforward=args.dff, dropout=args.dropout).to(device)
